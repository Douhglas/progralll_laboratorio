...
@NamedQueries({
    @NamedQuery(name = "TipoPlanilla.findAll", query = "SELECT t FROM TipoPlanilla t"),
     @NamedQuery(name = "TipoPlanilla.findByCodDesPla", query = "SELECT t FROM TipoPlanilla t WHERE UPPER(t.codigo) like :codigo and UPPER(t.descripcion) like :descripcion and UPPER(t.planillaPorMes) like :planillaPorMes", hints = @QueryHint(name = "eclipselink.refresh", value = "true")),
