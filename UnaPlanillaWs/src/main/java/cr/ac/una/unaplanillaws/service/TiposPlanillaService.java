...
@Stateless  
@LocalBean
public class TiposPlanillaService {

    private static final Logger LOG = Logger.getLogger(TiposPlanillaService.class.getName());
    ...
    public Respuesta getTiposPlanillas (String codigo, String descripcion, String planillaPorMes) {
        try {
            Query qryTipoPlanila = em.createNamedQuery("TipoPlanilla.findByCodDesPla", TipoPlanilla.class);
            qryTipoPlanila.setParameter("codigo", codigo);
            qryTipoPlanila.setParameter("descripcion", descripcion);
            qryTipoPlanila.setParameter("planillaPorMes", planillaPorMes);
            
            List<TipoPlanilla> tiposPlanilla = qryTipoPlanila.getResultList();
            ...
            return new Respuesta(true, CodigoRespuesta.CORRECTO, "", "", "TiposPlanillas", tipoPlanillasDto);

        } catch (NoResultException ex) {
            return new Respuesta(false, CodigoRespuesta.ERROR_NOENCONTRADO, "No existen tiposPlanilla con los criterios ingresados.", "getTiposPlanillas NoResultException");
        } catch (Exception ex) {
            LOG.log(Level.SEVERE, "Ocurrio un error al consultar el tipoPlanilla.", ex);
            return new Respuesta(false, CodigoRespuesta.ERROR_INTERNO, "Ocurrio un error al consultar el tipoPlanilla.", "getTipoPlanilla " + ex.getMessage());
        }
    }
