 ...
 */
public class SetVariablesWindowController extends Controller implements Initializable {
    ...
    private CreateNotProcessController createController;
    @FXML
    ...
    public void initialize() {
        ...
        createController = (CreateNotProcessController) FlowController.getInstance().getController("createNotProcess");

        //assigning the list of vars
        vars = createController.getVarList();
        notProcess = createController.getProcess();
    ...
    private void doneVarAction(ActionEvent event) {
        ...
        if (notProcess.getHtml().contains(vars.get(index))) {
            String value = "";
            ...
            notProcess.setContenido(notProcess.getContenido().replace("${" + vars.get(index) + "}", value));

            notProcess.updateHtml();
        ...
        if (index < vars.size()) {
            ...
            AppManager.getInstance().getProcesses().add(notProcess);
            AppManager.getInstance().saveProcess(notProcess);
            getStage().close();
            createController.getStage().close();
